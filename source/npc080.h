#pragma once

#include "npc.h"

void npcAct080(npc *NPC);
void npcAct081(npc *NPC);
void npcAct082(npc *NPC);
void npcAct083(npc *NPC);
void npcAct084(npc *NPC);
void npcAct085(npc *NPC);
void npcAct086(npc *NPC);
void npcAct087(npc *NPC);
void npcAct088(npc *NPC);
void npcAct089(npc *NPC);
void npcAct090(npc *NPC);
void npcAct091(npc *NPC);
void npcAct092(npc *NPC);
void npcAct093(npc *NPC);
void npcAct094(npc *NPC);
void npcAct095(npc *NPC);
void npcAct096(npc *NPC);
void npcAct097(npc *NPC);
void npcAct098(npc *NPC);
void npcAct099(npc *NPC);
