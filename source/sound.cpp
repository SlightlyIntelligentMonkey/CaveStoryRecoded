...
void mixAudioOrg(int *dst, Uint32 len)
{
	...
	if (org == nullptr || org->samplesPerStep == 0) { return; }

	if (isKeyDown(SDL_SCANCODE_UP)) { frequency *= 1.01; }
	if (isKeyDown(SDL_SCANCODE_DOWN)) { frequency *= 0.99; }
	...
	while ((currentPos << 2) < len)
	{
		...
		{
			if (org->samplesPerStep == 0) { return; }
